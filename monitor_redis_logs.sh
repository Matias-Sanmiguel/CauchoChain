...
detect_container() {
  ...
  local c
  c=$(docker ps --format '{{.Names}}' --filter ancestor=redis 2>/dev/null | head -n1 || true)
  if [ -n "$c" ]; then
    echo "$c"
    return
  fi
  c=$(docker ps --format '{{.Names}}' --filter name=redis 2>/dev/null | head -n1 || true)
  echo "$c"
}
...
run_redis_cmd() {
  ...
  if [ -n "${REDIS_CONTAINER:-}" ]; then
    docker exec -i "$REDIS_CONTAINER" redis-cli --raw "${password_arg[@]}" "$cmd" "$@"
  else
    redis-cli --raw "${password_arg[@]}" "$cmd" "$@"
  fi
}
